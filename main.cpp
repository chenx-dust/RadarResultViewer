...
std::map<size_t, cv::Point2d> Enemies;
...
void draw_point(cv::Point2d pt, int tag)
{
    std::cout << "Draw point: " << pt << " " << tag << std::endl;
    cv::Scalar color;
    size_t r_tag;
    if (tag < 100)  // 红
    ...
    cv::Point center(GroundSize.x * pt.x / REAL_SIZE.x, GroundSize.y * (1 - pt.y / REAL_SIZE.y));
    cv::circle(DrawedGround, center, 10, color / 2, -1);
    cv::circle(DrawedGround, center, 10, color, 1);
    cv::putText(DrawedGround, std::to_string(r_tag), center + cv::Point(-6, 4), cv::FONT_HERSHEY_SIMPLEX, 0.5, { 255, 255, 255 }, 1);
}
